...
## Build in Public Log Template

Use this format for GitHub updates, LinkedIn posts, or commit notes:

```text
Today I worked on: <feature or pattern>

Problem:
<what could break in a distributed system>

Solution:
<what pattern was added and why>

What I learned:
<1-3 concrete lessons>
...
```text
Today I worked on request idempotency for transaction creation.

Problem:
If a client retries the same POST request because of a timeout, the platform could create two transactions and reserve tokens twice.

Solution:
I added an idempotency key flow so the same request can return the original transaction instead of creating a duplicate.

What I learned:
Idempotency is not only "check if a row exists". It also needs user scoping, request fingerprint validation, and concurrency handling.

Next:
Add integration tests for duplicate requests and same key with different payload.
```
